...
import {
  coreUpdateQueryOptions,
  DEFAULT_CORE_UPDATE_POLL_INTERVAL_MS,
  healthQueryOptions,
  instancesQueryOptions,
  systemQueryOptions,
  tasksQueryOptions,
} from "../api/queries"
import { CoreUpdatePanel } from "../core-update/core-update-panel"
...
import { useInstanceLifecycleMutation } from "./hooks/use-instance-lifecycle-mutation"
...
export interface DashboardProps {
  ...
  coreUpdatePollIntervalMs?: number
...
export function Dashboard({
  ...
  activeView,
  expandedInstance,
  coreUpdatePollIntervalMs = DEFAULT_CORE_UPDATE_POLL_INTERVAL_MS,
  onExpandedInstanceChange,
  onNavigate,
}: DashboardProps) {
  const [selectedInstance, setSelectedInstance] = useState<string | null>(null)
  const lifecycleMutation = useInstanceLifecycleMutation(api)
  ...
  useQuery(coreUpdateQueryOptions(api, coreUpdatePollIntervalMs))
  ...
  return (
    <div className="app-viewport overflow-hidden text-slate-950">
      <div className="mx-auto grid h-full max-w-385 grid-rows-[auto_minmax(0,1fr)] overflow-hidden rounded-4xl border border-white/70 bg-white/48 shadow-[0_28px_80px_-34px_rgba(30,64,83,0.45)] backdrop-blur-[34px] lg:grid-cols-[15rem_minmax(0,1fr)] lg:grid-rows-1 lg:rounded-[2.25rem]">
        <DashboardSidebar
          ...
          updateActive={activeView.kind === "update"}
          ...
          onSelectUpdate={() => onNavigate({ kind: "update" })}
          onSelectInstance={toggleInstance}
          onSelectTab={showInstance}
          onToggleInstanceRunning={(instance) =>
            lifecycleMutation.mutate({
              instance: instance.name,
              action: instance.running ? "stop" : "start",
            })
          }
          pendingInstanceAction={
            lifecycleMutation.isPending ? lifecycleMutation.variables.instance : undefined
          }
        />

        {activeView.kind === "dashboard" ? (
          <main
            className="min-h-0 min-w-0 overflow-y-auto overscroll-contain bg-[rgba(248,251,252,0.34)] p-3 sm:p-4 lg:p-8"
            id="overview"
          >
            <div className="mx-auto w-full max-w-5xl">
              ...
              <section className="relative mt-6 overflow-hidden rounded-[1.75rem] bg-[#174c68] p-5 text-white ring-1 ring-white/20 sm:p-7 lg:p-8">
                  ...
                  <div className="border-slate-900/6 border-t xl:max-h-128 xl:overflow-y-auto xl:overscroll-contain">
                    ...
                    {tasks.isError ? (
                      <EmptyState
                        ...
                        title="任务读取失败"
                        detail={
                          tasks.error instanceof Error ? tasks.error.message : "稍后刷新重试。"
                        }
                      />
                    ) : null}
        ...
        ) : activeView.kind === "update" ? (
          <CoreUpdatePanel api={api} />
        ) : (
          <InstanceDetail
